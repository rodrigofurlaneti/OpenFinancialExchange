...
public sealed class BankAccount : AggregateRoot
{
    ...
    private BankAccount(long financialInstitutionId, string bankId, string? branchId, string acctId, string acctType) : base(0)
    {
    ...
    public static Result<BankAccount> Create(
        long financialInstitutionId, string bankId, string? branchId, string acctId, string acctType)
    {
        ...
        return Result.Success(new BankAccount(
            financialInstitutionId, bankId.Trim(), branchId?.Trim(), acctId.Trim(), acctType!.ToUpperInvariant()));
    }
